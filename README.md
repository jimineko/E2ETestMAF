...
Microsoft Agent Framework (MAF) と Microsoft公式 Playwright MCP を使い、Webアプリケーションの探索、テスト生成、ブラウザ実行、判定、再試行、レポート作成を行う、型付きの自律型E2Eテスト製品です。チャットモデルは Azure OpenAI / Google Gemini / GitHub Copilot (GitHub Models経由) を選択できます。
...
GitHub Copilot（GitHub Models）を使う場合は次を設定します。`MAF_E2E_GITHUB_COPILOT_TOKEN` を省略した場合、`MAF_E2E_GITHUB_COPILOT_USE_GH_CLI_TOKEN=true` で `gh auth token` を使用します。

```dotenv
MAF_E2E_MODEL_PROVIDER=github_copilot
MAF_E2E_GITHUB_COPILOT_MODEL=gpt-4.1
MAF_E2E_GITHUB_COPILOT_BASE_URL=https://models.inference.ai.azure.com
MAF_E2E_GITHUB_COPILOT_TOKEN=YOUR_GITHUB_TOKEN
```
...
MAF_E2E_MODEL_PROVIDER=gemini
MAF_E2E_GEMINI_MODEL=gemini-2.5-flash-lite
MAF_E2E_GEMINI_USE_VERTEX_AI=true
